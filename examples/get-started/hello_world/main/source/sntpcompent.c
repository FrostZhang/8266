...
const char *sntptag = "sntpcompent";
time_t now = 0;
struct tm timeinfo = {0};

static void initialize_sntp(void)
{
    ESP_LOGI(sntptag, "Initializing SNTP");
...
static void obtain_time(void)
{
    ...
    {
        ESP_LOGI(sntptag, "Waiting for system time to be set... (%d/%d)", retry, retry_count);
...
static void sntp_task(void *arg)
{
    ...
    {
        ESP_LOGI(sntptag, "Time is not set yet. Connecting to WiFi and getting time over NTP.");
        obtain_time();
    }
    ...
    if (timeinfo.tm_year < (2016 - 1900))
    {
        ESP_LOGE(sntptag, "The current date/time error");
    ...
    else
    {
        ...
        ESP_LOGI(sntptag, "The current date/time in Shanghai is: %s", strftime_buf);
        //ESP_LOGI(sntptag, "Free heap size: %d\n", esp_get_free_heap_size());
